...
enum Speaker_States{START_Speaker, ON} Speaker;
void speaker_state(){
	unsigned char tempA = ~PINA & 0x04;
	switch(Speaker)
	{
		case START_Speaker:
			Speaker = ON;
...
void combineStates(){
	...
	switch(CombineLEDsSM)
	{
		...
		case TLBL_SMStart:
			...
			break;
		case OUTPUT:
			PORTB = (PORTB &  0xE0) | threeLEDs | blinkingLED | sound;
			break;
		default: break;
	}
...
int main(void) {
    ...
    while (1) {
     	...
     	combineStates(); //100 was the GCD for this so it always works
